...
export class NodeNoteSettingsDialog extends NodeNoteDialog {
    constructor(app) {
        super();
		this.app = app;
		this.settingsValues = {};
        ...
        this.element = $el(
			"dialog",
			{
				id: "nodenote-settings-dialog",
				parent: document.body,
			},
			[
				$el("table.nodenote-modal-content.nodenote-table", [
					$el(
						"caption",
						{ textContent: "Settings" },
						$el("button.nodenote-btn", {
							type: "button",
							textContent: "\u00d7",
							onclick: () => {
								this.element.close();
	...
	addSetting({ id, name, type, defaultValue, onChange, attrs = {}, tooltip = "", options = undefined }) {
		...
		if (value == null) {
			if (this.app.isNewUserSession) {
				...
				const localValue = localStorage["NodeNote.Settings." + id];
				if (localValue) {
		...
		this.settingsLookup[id] = {
			...
			render: () => {
				...
				const labelCell = $el("td", [
					$el("label", {
						for: htmlID,
						classList: [tooltip !== "" ? "nodenote-tooltip-indicator" : ""],
						textContent: name,
					}),
				]);
